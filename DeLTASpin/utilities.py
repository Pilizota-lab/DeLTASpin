...
def label_seg_extr(
    seg: SegmentationMask,
    cellnumbers: List[int] = None,
    cellstoremove: List[int] = None,
    background: int = 0,
) -> Union[Labels, Tuple[Labels, List[Contour]]]:
    """
    Label cells in segmentation mask -> during extraction!!!!
    ...
    count = 0 #initialise count to remove
    ...
    label = np.full(seg.shape, background, dtype=np.uint16)
    for c, contour in enumerate(contours):
        if c in cellstoremove:
            #remove this cell, and keep in mind that it has been removed
            count += 1
        else:
            label = cv2.fillPoly(
                label, [contour], cellnumbers[c-count] #cellnumbers will be shifted by the number of cells that were not saved (i.e., count)
            )
...
class Lineage:
    ...
    def __init__(self):
        ...
        self.notsaved = []
        #list of contours in each frame that are not actually saved as cells
    ...
    def update(
        ...
        if len(self.notsaved) <= frame+1:
            self.notsaved += [
                [] for _ in range(frame + 2 - len(self.notsaved)) 
            ]  # Initialize -> add empty list at index [frame]
        if frame == 0: #initialise self.cellnumbers for first frame -> every time it goes through a cell, increase length of self.cellnumbers[0] by 1
            self.cellnumbers[0] += [-1 for _ in range(max(attrib) + 1 - len(self.cellnumbers[0]))]
        if len(self.cellnumbers[frame]) == 0 and frame!=0: #initialise for subsequent frames
            self.cellnumbers[frame] += [-1 for _ in self.cellnumbers[0]]  # Initialize just in case cell needs to be added, but get rid at the end if not needed

        # If no attribution: (new/orphan cell)
        if cell is None and len(attrib) > 0: #attribute was given the index of the cell in current frame in pipeline.py
            if frame == 0:
                ...
                # print(self.cellnumbers[frame])
            # else:
                # save which cell will not be saved
                # print(attrib[0], "cell trying to be created",  self.cellnumbers[frame])

        # Simple tracking event:
        elif len(attrib) == 1:
            cellnum = self.cellnumbers[0][cell]  # Get old cell number (from frame 0)
            # Update cellnumbers list:
            # find out how many cells in current frame before attrib[0] have been notsaved
            notsavedcurrent = sum(1 for x in self.notsaved[frame] if x < attrib[0])
            self.cellnumbers[frame][attrib[0] - notsavedcurrent] = cellnum
            # print(frame, attrib[0], notsavedcurrent)
            # find latest frame where the cell was seen
            for latest_frame in range(frame-1, -1, -1): #check frames from frame-1 up to and including 0
                if cellnum in self.cellnumbers[latest_frame]:
                    break
            # Poles of mother/previous/old cell:
            prev_old = self.getvalue(cellnum, latest_frame, "old_pole")
            prev_new = self.getvalue(cellnum, latest_frame, "new_pole")
        ...
        # Division event:
            ...
            prev_new = self.getvalue(mothernum, 0, "new_pole")
            assert isinstance(prev_old, np.ndarray)  # for mypy
            assert isinstance(prev_new, np.ndarray)  # for mypy
            notsavedcurrent = sum(1 for x in self.notsaved[frame] if x < attrib[0])
            ...
            # Update cellnumbers list:
            # self.cellnumbers[frame] += [-1]
            if first_cell_is_mother:
                self.notsaved[frame] += [attrib[1]]
                self.cellnumbers[frame][attrib[0] - notsavedcurrent] = mothernum
                # self.cellnumbers[frame][attrib[1]] = daughternum
            else:
                self.notsaved[frame] += [attrib[0]]
                # self.cellnumbers[frame] += [-1]
                # self.cellnumbers[frame][attrib[0]] = daughternum
                self.cellnumbers[frame][attrib[1] - notsavedcurrent] = mothernum
...
def vidwrite(
    images: np.ndarray, filename: Union[str, Path], crf: int = 28, verbose: int = 1 #change
) -> None:
    """
