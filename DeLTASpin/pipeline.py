...
class Position:
    ...
    def legacysave(self, res_file: Union[str, Path]) -> None:
        ...
        # Lineages: - changed (irrelevant data commented out)
        data["res"] = []
        for r in self.rois:
...
class ROI:
    ...
    def get_tracking_inputs(
        ...
        if (
            ...
            for c in range(len(poles)): #this is for first frame
                self.lineage.update(None, frame, attrib=[c], poles=[poles[c]])

            return None, None

        # Otherwise, get cell contours:
        cells = utils.find_contours(self.seg_stack[0]) #changed
        ...
        for c, cell in enumerate(cells):
            ...
            if self.crop_windows:
                ...
                prev_img = self.img_stack[0] #changed
                ...
                prev_img = cv2.resize(
                    self.img_stack[0], dsize=cfg.target_size_seg[::-1] #changed
                )
    ...
    def process_tracking_outputs(
        ...
        if scores is None:
            self.lineage.update(None, frame) #this will only happen if no cells found in current FoV
            return
        ...
        # Update lineage: - this is once initial cells have already been created
        # Go through "orphan" cells:
        for n in range(attributions.shape[1]):
            attrib = attributions[:, n].nonzero()[0] #takes all non-0 values
            # new_cells_poles = [poles[n]]
            if len(attrib) == 0: #only 0-values in attributions, so not linked to any cells in previous frame
                # print("attrib was none")
                self.lineage.notsaved[frame] += [n]
            # if len(attrib) == 0:
        ...
        for o in range(attributions.shape[0]):
            ...
            new_cells_poles = []
            for n in attrib:
                new_cells_poles += [poles[n]] # n represents the actual index of the cell in current frame to be assigned
            self.lineage.update(o, frame, attrib=attrib, poles=new_cells_poles) #this includes division!!
    ...
    def extract_features(
        ...
        if self.label_stack[frame] is None:
            if len(self.lineage.cellnumbers) <= frame:
                ...
                cell_nbs = [c + 1 for c in self.lineage.cellnumbers[frame]] #change this to first get rid of the cellnumbers[frame] that are set to -2 (so will turn into -1)
            labels = utils.label_seg_extr(self.seg_stack[frame], cell_nbs, self.lineage.notsaved[frame])
            assert isinstance(labels, np.ndarray)  # needed for mypy on Python < 3.8
